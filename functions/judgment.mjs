...
export function evaluateJudgment({records,goal='',criteria=null,cases=[]}){
 ...
 const validPlan=matching&&plan?.program?.length&&date&&plan.savedAt>0&&new Date(plan.savedAt).toISOString().slice(0,10)<date&&actual.length;
...
각 카드에 대해 judgments를 정확히 한 개씩 반환하고 cardId/version/status를 그대로 유지한다. needs_information은 부족한 필드와 확인 행동을 설명하고 진행/정체/목표 성공으로 바꾸지 않는다. not_applicable은 왜 적용하지 않는지 짧게 설명한다.
관찰(observation), 해석 가설(interpretation), 판단을 바꿀 질문(question), 트레이너의 검토 행동(action)을 분리한다. 수치는 cards.metrics와 summary에 있는 값만 사용한다. 근거 ID는 카드의 evidenceIds 안에서 인용한다.
