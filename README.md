...
서버 모드 실제 회원 화면은 원본과 작업 영역의 50:50 2뷰이며, 가운데 `기록 수정 / 진행 분석 / 다음 수업` 탭으로 전환합니다. 3뷰에서는 오른쪽 AI 도우미를 추가합니다. 모바일에서는 원본과 작업 영역을 위아래로 표시합니다. 상세 구성은 `docs/source-workspace-next.md`에 정리했습니다. 업로드 성공 시 업로드 창을 닫고 진행 분석을 표시합니다. 판독/분석은 완료 순서대로 실시간 반영됩니다.
