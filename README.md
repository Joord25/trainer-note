...
### 회원 홈과 업로드 후 분석 (2026-09-12)

로그인/재접속은 회원 카드 목록으로 시작합니다. 회원을 선택하면 분석 작업실이 열리며, 사이드바의 회원 목록으로 홈에 돌아올 수 있습니다.

사용자가 원한 기본 동선은 **업로드 → 자동 판독 → 진행 분석을 먼저 표시**입니다. 기록 입력/확정 목록을 통과하는 단계는 두지 않습니다.
서버 모드 실제 회원 화면은 원본과 작업 영역의 50:50 2뷰로 시작하며 구분선을 드래그해 너비를 조절할 수 있고, 가운데 `기록 수정 / 진행 분석 / 다음 수업` 탭으로 전환합니다. 3뷰에서는 오른쪽 AI 도우미를 추가합니다. 모바일에서는 원본과 작업 영역을 위아래로 표시합니다. 상세 구성은 `docs/source-workspace-next.md`에 정리했습니다. 업로드 성공 시 업로드 창을 닫고 진행 분석을 표시합니다. 판독/분석은 완료 순서대로 실시간 반영됩니다.
