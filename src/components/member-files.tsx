...
export function MemberFiles({memberId,memberName,online}:{memberId:string;memberName:string;online:boolean}) {
  ...
  return <section className="member-files" aria-label="회원 운동일지">
    <div className="file-section-heading"><div><h2>운동일지 <span>{files.length}</span></h2><p>{memberName} 회원에게 연결된 원본 파일이에요.</p></div><button className="primary" disabled={!enabled} onClick={()=>picker.current?.click()}><Icon name="plus" size={16}/> 파일 추가</button></div>
