...
const defaults={two:[50,50],three:[36.76,36.76,26.48],collapsed:[72,28]};
...
export function ResizableReviewColumns({mode,sourceHidden,children}:{mode:number;sourceHidden:boolean;children:ReactNode}){
 ...
 const [viewport,setViewport]=useState(0),[sizes,setSizes]=useState(defaults),[resizing,setResizing]=useState(false);
 ...
 const enabled=viewport>760&&(!sourceHidden||assistant),values=sizes[layout];
 ...
 function change(index:number,delta:number,base:number[],width:number,key:Layout){
  ...
  const next=[...base];next[index]=left;next[index+1]=sum-left;setSizes(prev=>({...prev,[key]:next}));
 }
 ...
 function key(e:KeyboardEvent<HTMLDivElement>,index:number){if(e.key==='Enter'){e.preventDefault();setSizes(v=>({...v,[layout]:defaults[layout]}));return;}if(e.key!=='ArrowLeft'&&e.key!=='ArrowRight')return;e.preventDefault();change(index,(e.key==='ArrowLeft'?-1:1)*(e.shiftKey?5:2),values,root.current?.clientWidth||1,layout);}
