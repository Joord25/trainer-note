    ...
    <main className="main-workspace real-main">
      ...
      <div className="real-content">
        ...
        {error ? <div className="member-error" role="alert"><h2>회원 목록을 불러오지 못했어요</h2><p>{error}</p><button onClick={() => setAttempt(v => v + 1)}>다시 시도</button></div> : loading ? <div className="empty-state" aria-busy="true"><span className="auth-spinner"/><p>{online ? "회원 정보를 불러오고 있어요." : "인터넷 연결을 기다리고 있어요."}</p></div> : !selected ? <section className="member-welcome">
          ...
          <MemberRecords key={selected.id+"-records"} memberId={selected.id} memberName={selected.name} online={online}/>
          <MemberFiles key={selected.id} memberId={selected.id} memberName={selected.name} online={online}/>
          <div className="member-bottom-actions"><p>{(selected.fileCount > 0 || selected.recordCount > 0) ? "회원을 삭제하려면 연결된 PDF와 운동 기록을 먼저 삭제해주세요." : "회원 정보는 로그인한 트레이너 계정에 저장돼요."}</p><button className="member-delete-button" onClick={() => setEditor({kind: "delete", member: selected})} disabled={!available || selected.pending || (selected.fileCount > 0 || selected.recordCount > 0)} title={(selected.fileCount || selected.recordCount) ? "연결된 PDF와 운동 기록을 먼저 삭제해주세요." : undefined}>회원 삭제</button></div>
        </>}
      </div>
    </main>
