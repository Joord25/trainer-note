...
export const EXTRACTION_VERSION = 'workout-v1';
export const MAX_AI_BYTES = 10 * 1024 * 1024;
export type ExtractedWorkout = {input:WorkoutInput;issues:string[];memberName:string;id:string};
...
시간/거리/라운드/LR/슈퍼세트 등 반복 횟수로 확정할 수 없는 항목은 kg·회로 억지 변환하지 말고 unparsed에 원문과 이유를 기록한다. 워밍업·쿨다운의 실제 기록도 동일하게 처리한다.
...
export async function parseExtraction(value:unknown,source:{id:string;name:string;contentType:string},memberName:string,year?:number):Promise<Extraction>{
 ...
 const records=await Promise.all(array(root.records,60).map(async item=>{
  ...
  return {id,memberName:name,issues:[...new Set(issues)],input:{date,rawName,exerciseName,bodyPart,loadType:loadType as WorkoutInput['loadType'],sets,sourceHash:source.id,sourceName:source.name,sourcePage:page,notes}};
 }));
