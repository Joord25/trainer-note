...
export type Analysis={status:'queued'|'processing'|'ready'|'error'|'limited';fingerprint?:string;startedAt?:{toMillis:()=>number};error?:string;excludedCount?:number;scopeLimit?:number;summary?:{days:number;sets:number;volume:number;autoRecords:number;parts:Record<string,number>;trend:{date:string;sets:number;volume:number}[]};report?:{headline:string;overview:string;findings:{title:string;detail:string;evidenceIds:string[]}[];limitations:string[];questions:string[];program:PlanRow[];quests:string[]}};
